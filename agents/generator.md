...
- Code, document, and configuration generation
...
### Input
- `state/session.json` → `parsed`, `team`, `tasks`, `skills`
- `state/session.json` → `research_output` (if researcher was in team, otherwise null)
...
- If `research_output` contains `data_tables`, plan files to present them
...
2. Write each file content based on file_plan and research_output
3. After each file, log: `✅ wrote [filename]`
4. Update `state/session.json` `files_written[]` with `{ "path": "<path>", "lines": <count> }` after each file

### Content Rules
- CLAUDE.md: ONLY routing/prompt logic, zero documentation
- Agent .md: Must define role, max_turns (hardcap 3), I/O, behavior
...
- Log all errors to `state/session.json` `errors[]` with `{ "step": <5|6>, "agent": "generator", "message": "<error>" }`
